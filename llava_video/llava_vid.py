        ...
        **kwargs,
        ...
        if self.overwrite == True:
            ...
            if "v1.5" in pretrained:  # A hardcode solution here to load v1.5 model, otherwise it will use LlavaConfig from hf transformers
                ...
                if self.device_map != "auto" and self.device_map != "balanced_low_0":
                ...
                attn_implementation = None # Optinal : None, "sdpa", "flash_attention_2"
                self._tokenizer, self._model, self._image_processor, self._max_length = load_pretrained_model(pretrained, None, self.model_name, device_map=self.device_map, overwrite_config=overwrite_config, attn_implementation=attn_implementation)
        else:
            attn_implementation = None # Optinal : None, "sdpa", "flash_attention_2"
...
#                         json.dump({}, f)  # 先写入一个空的 JSON 结构
            ...
            save_cluster_root_path = 'SET-ROOT-PATH-HERE' + self.model_name +'_' + str(self.max_frames_num) + 'f_' + str(num_classes_total) + 'classes_' + str(rho) + 'rho_' + str(eps) + 'eps'
            ...
            if cluster_method is not None:
                with torch.inference_mode():
                    output_ids = self.model.generate(
                        ...
                        rho=rho,
                        eps=eps,
                        save_cluster_path = save_cluster_path, # new code
                    )
