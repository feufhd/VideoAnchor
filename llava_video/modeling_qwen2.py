...
def my_softmax(x, attention_boost_q=None, attention_boost_k=None):
    
    if attention_boost_q is not None:
        log_boost_q = torch.log(attention_boost_q + 1e-6).view(1, 1, -1, 1)  # shape: (1, 1, Q, 1)
        x = x + log_boost_q

    if attention_boost_k is not None:
        log_boost_k = torch.log(attention_boost_k + 1e-6).view(1, 1, 1, -1)  # shape: (1, 1, 1, K)
        x = x + log_boost_k
...
class Qwen2Attention(nn.Module):
    ...
    def forward(
        ...
        if self.layer_idx == self.config.num_hidden_layers - 1 and self.num_heads == 28 and self.hidden_size == 3584:
            ...
            attn_weights = torch.matmul(query_states.to(torch.float32) / math.sqrt(self.head_dim), key_states.transpose(2, 3).to(torch.float32))
        else:
            attn_weights = torch.matmul(query_states.to(torch.float32), key_states.transpose(2, 3).to(torch.float32)) / math.sqrt(self.head_dim)
        ...
        if attention_boost_k is not None:
            ...
            if attention_boost_k.device != attn_weights.device:
                ...
                attn_weights = my_softmax(attn_weights, attention_boost_q, attention_boost_k).to(query_states.dtype)
        ...
        if attention_boost_v is not None:
            if attention_boost_v.device != value_states.device:
                value_states = value_states * (attention_boost_v.view(1, 1, attention_boost_v.shape[0], 1).to(value_states.device))
            else:
                value_states = value_states * attention_boost_v.view(1, 1, attention_boost_v.shape[0], 1)
            
            del attention_boost_q
            del attention_boost_k
            del attention_boost_v
...
class Qwen2SdpaAttention(Qwen2Attention):
    ...
    def forward(
        ...
        if attention_boost_k is not None:
            if key_states.shape[-2] != attention_boost_k.shape[0]:
                # 生成阶段：拼接一个 1.0 给新 token
                attention_boost_q = torch.cat([attention_boost_q, torch.ones(key_states.shape[-2] - attention_boost_q.shape[0], device=attention_boost_q.device)])
                attention_boost_k = torch.cat([attention_boost_k, torch.ones(key_states.shape[-2] - attention_boost_k.shape[0], device=attention_boost_k.device)])
                attention_boost_v = torch.cat([attention_boost_v, torch.ones(key_states.shape[-2] - attention_boost_v.shape[0], device=attention_boost_v.device)])
            
            if attention_boost_k.device != key_states.device:
                attention_boost_q = attention_boost_q.to(key_states.device)
                attention_boost_k = attention_boost_k.to(key_states.device)
                attention_boost_v = attention_boost_v.to(key_states.device)
                
            if causal_mask is not None:
                causal_mask = causal_mask + torch.log(attention_boost_q + 1e-6).view(1, 1, -1, 1) + torch.log(attention_boost_k + 1e-6).view(1, 1, 1, -1)
            else:
                causal_mask = torch.zeros((1, 1, attention_boost_q.shape[0], attention_boost_k.shape[0]), device=attention_boost_q.device) + torch.log(attention_boost_q + 1e-6).view(1, 1, -1, 1) + torch.log(attention_boost_k + 1e-6).view(1, 1, 1, -1)
...
class Qwen2DecoderLayer(nn.Module):
    def __init__(self, config: Qwen2Config, layer_idx: int):
        ...
        # new code
        config._attn_implementation = "eager" #'eager'
...
class Qwen2ForCausalLM(Qwen2PreTrainedModel):
    ...
    def prepare_inputs_for_generation(
        ...
        **kwargs,
        ...
        attention_boost_q = kwargs.get("attention_boost_q", None) # new add
        ...
        model_inputs.update(
            {
                ...
                "use_cache": use_cache,
                "attention_mask": attention_mask,
                "attention_boost_q": attention_boost_q,  # new code ✅ 把它送进 forward!
