...
class Qwen2_5_VLFlashAttention2(Qwen2_5_VLAttention):
    ...
    def forward(
        ...
        if attention_boost_v is not None:
            if value_states.shape[2] != attention_boost_v.shape[0]:
                # 生成阶段：拼接一个 1.0 给新 token
                attention_boost_v = torch.cat([attention_boost_v, torch.ones(value_states.shape[2] - attention_boost_v.shape[0], device=attention_boost_v.device)])
            if attention_boost_v.device != value_states.device:
                value_states = value_states * (attention_boost_v.view(1, 1, attention_boost_v.shape[0], 1).to(value_states.device))
            else:
                value_states = value_states * attention_boost_v.view(1, 1, attention_boost_v.shape[0], 1)
...
class Qwen2_5_VLModel(Qwen2_5_VLPreTrainedModel):
    def __init__(self, config: Qwen2_5_VLConfig):  
        config._attn_implementation = 'eager' # new
...
class Qwen2_5_VLForConditionalGeneration(Qwen2_5_VLPreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        if (cluster_method == 'ssc_4') and (inputs_embeds.shape[1] > 1):
            ...
            labels_cluster = c0 #torch.tensor(c0) # torch.tensor([...], dtype=torch.long)  # shape: (2048,)

            # 1. 聚类计数
            class_counts = (torch.bincount(labels_cluster, minlength=num_classes_total) / labels_cluster.shape[0]) #.to(relation_sum.device)

            relation_sum = (relation_sum * class_counts[labels_cluster]).to(inputs_embeds.device)
